...
// ═══════════════════════════════════════════════════════════════
// Aggregates QuickTrades + Polymarket positions for the POSITION tab.
...
/** Total USDC P&L (Polymarket only — real money) */
export const totalPnlUsdc = derived(unifiedPositions, ($p) =>
  $p
    .filter((p) => p.type === 'polymarket' && p.pnlUsdc != null)
 ...
 */
export async function hydratePositions(type?: 'all' | 'quick_trade' | 'polymarket'): Promise<void> {
