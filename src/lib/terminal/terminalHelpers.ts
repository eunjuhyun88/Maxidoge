...
export function getDefaultTabletIntelWidth(viewportWidth?: number): number {
  if (!Number.isFinite(viewportWidth)) return 320;
  return Math.round(Math.min(400, Math.max(240, (viewportWidth as number) * 0.35)));
}

export function clampTabletIntelWidth(next: number, viewportWidth?: number): number {
  const min = 200;
  const max = 480;
  if (!Number.isFinite(viewportWidth)) {
    return Math.round(Math.min(max, Math.max(min, next)));
  }
  const dynamicMax = Math.min(max, Math.max(280, Math.round((viewportWidth as number) * 0.5)));
  return Math.round(Math.min(dynamicMax, Math.max(min, next)));
}
