<script lang="ts">
  ...
  import {
    ...
    type PatternScanScope,
    type PatternScanResult,
  } from '$lib/engine/patternOverlay';
  ...
  function applyDetectedPatternState(next: ChartPatternDetection[]) {
    const signature = buildPatternSignature(next);
  ...
  function runPatternDetection(
    ...
    let sourceCandles = scope === 'visible' ? getVisibleScopeCandles(chart, klineCache) : klineCache;
...
<style>
  ...
  .scan-btn:hover {
    background: linear-gradient(135deg, rgba(255,230,0,.28), rgba(255,180,0,.18));
    border-color: rgba(255,230,0,.55);
    color: #fff3bf;
  ...
  .pos-sl { color: #ff4060; }
  .pos-rr { font-size: 10px; font-weight: 900; font-family: var(--fd); color: #ffe600; background: rgba(0,0,0,.6); padding: 2px 8px; border-radius: 4px; }
