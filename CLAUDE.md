...
| `/passport` | 유저 프로필 허브 — 보유, 트레이드, 시그널, 에이전트, ORPO 학습 | userProfileStore, userLifecycleStore, matchHistoryStore, quickTradeStore, agentStats | 520 |
...
- `src/components/arena/chart/chartDrawingPersistenceRuntime.ts`: ChartPanel drawing stack의 lazy module load, primitive drawing autosave, pair/timeframe persistence sync 계층
- `src/components/arena/chart/chartTradePreviewFinalizeRuntime.ts`: ChartPanel의 line-entry/trade-preview finalize lazy runtime; preview resolve, tradebox drawing 생성, quick-trade/plan finalize side effect를 mouse-up 시점에만 로드
- `src/lib/chart/primitives/drawingManagerTypes.ts`: Chart primitive drawing stack의 shared serialization/type contract 계층; `DrawingData`, `CandleOHLC`, `DrawingManagerCallbacks`를 보유
- `src/lib/chart/primitives/drawingPrimitiveRegistry.ts`: Chart primitive drawing stack의 factory/snapshot registry 계층; primitive class import와 serialized restore/snapshot logic을 `DrawingManager` 밖으로 분리
- `src/lib/chart/primitives/drawingPositionPrimitiveRegistry.ts`: Chart primitive drawing stack의 position-entry lazy registry 계층; `PositionPrimitive` preview/restore 생성 경로를 base registry에서 분리
...
- `src/components/arena/chart/chartActionRuntime.ts`: ChartPanel의 pair/timeframe 전환과 chart-origin scan immediate action 계층; interactive chart actions는 lazy runtime으로 위임
- `src/components/arena/chart/chartInteractiveActionRuntime.ts`: ChartPanel의 interactive chart action lazy runtime; chat assist, community signal publish, trade-drawing activation을 첫 user action 시점에만 로드
...
- `src/components/arena/chart/chartToolbarCatalog.ts`: ChartPanel drawing toolbar의 tool category/icon catalog 경계; toolbar가 category/tool metadata를 인라인 중복하지 않도록 고정
- `src/components/arena/chart/chartAgentOverlayChromeContracts.ts`: ChartPanel agent overlay chrome prop contract 계층; parent/composition child/meta child/action child가 동일 계약을 공유
- `src/components/arena/chart/ChartAgentOverlayChrome.svelte`: ChartPanel의 agent overlay chrome composition boundary; meta/action child를 조립해 기존 overlay stacking order를 유지
- `src/components/arena/chart/ChartAgentMetaOverlay.svelte`: ChartPanel agent overlay의 scale tools/indicator legend/loading/error child boundary
- `src/components/arena/chart/ChartAgentActionOverlay.svelte`: ChartPanel agent overlay action composition boundary; trade/status HUD child를 조립해 existing action-stack order를 유지
- `src/components/arena/chart/ChartAgentTradeHud.svelte`: ChartPanel agent overlay action stack의 close button/first-scan CTA/trade CTA child boundary
- `src/components/arena/chart/ChartAgentStatusHud.svelte`: ChartPanel agent overlay action stack의 drawing/position HUD composition boundary; status child render order와 shared prop handoff를 고정
- `src/components/arena/chart/ChartAgentDrawingStatusHud.svelte`: ChartPanel agent overlay action stack의 drawing notice/chart notice/drag indicator child boundary
- `src/components/arena/chart/ChartAgentPositionHud.svelte`: ChartPanel agent overlay action stack의 position sizer dock/position badge/risk-reward HUD child boundary
...
- `src/components/arena/chart/ChartAgentSurface.svelte`: ChartPanel의 chart-container surface child boundary; chart container DOM/class/event surface와 overlay stacking order를 child가 보유하고, optional `ChartTradePlanOverlay`/`ChartDrawingContextMenu` lazy boundary를 소유
...
- `src/components/arena/chart/chartClientRuntime.ts`: ChartPanel의 client-only lazy import/support-runtime/controller/derivatives assembly 경계; ChartPanel이 module-loading/instantiation 순서를 직접 소유하지 않도록 고정
- `src/components/arena/chart/chartPanelMountState.ts`: ChartPanel의 prepared-mount/bootstrap state apply 경계; mount 결과를 component local state에 주입하는 순서와 MA binding 생성 순서를 고정
- `src/components/arena/chart/chartPanelStateContext.ts`: ChartPanel의 indicator series ref/pane index projection, data-runtime series context, indicator accumulator snapshot read 경계
- `src/components/arena/chart/chartPanelRuntimeOptions.ts`: ChartPanel의 runtime-bundle position/tradingView/data/bindings option 조립 경계; 패널이 거대한 option shape를 직접 손으로 복제하지 않도록 고정
...
- `src/lib/chart/chartTradePlannerMath.ts`: ChartPanel trade-plan/community-signal 경계가 공유하는 reward-ratio/risk-percent 순수 계산 계층
- `src/lib/chart/chartCommunitySignalDraft.ts`: ChartPanel community signal draft lazy assembly 경계; chart action 시점에만 dynamic import되는 chart-origin signal payload 조립 계층
...
### 5. Passport (유저 프로필 — `/passport`, 520줄)
...
- 핵심 파일: `passport/+page.svelte`, `components/passport/PassportHeaderSection.svelte`, `components/passport/PassportNavChrome.svelte`, `components/passport/PassportProfileTab.svelte`, `components/passport/PassportWalletTab.svelte`, `components/passport/PassportPositionsTab.svelte`, `components/passport/PassportArenaTab.svelte`, `components/passport/PassportLearningPanel.svelte`, `components/passport/passportTabShared.css`, `lib/passport/passportSummaryViewModel.ts`, `lib/passport/passportHoldingsRuntime.ts`, `userProfileStore`, `progressionRules`
...
| Passport (`/passport`) | 🔶 구조 분해 진행중 | 520줄 — header/nav + 4 tab hosts 분리, summary/holdings/learning runtime 추출 완료 |
...
- **passport profile tab canonical path**: profile 탭의 performance snapshot, extra metrics, badges surface는 `src/components/passport/PassportProfileTab.svelte`가 canonical이다. route에 profile tab markup/CSS를 다시 인라인하지 말 것.
- **passport wallet tab canonical path**: wallet 탭의 virtual balance, holdings donut/table, holdings live badge는 `src/components/passport/PassportWalletTab.svelte`가 canonical이다. route나 header/nav shell에 wallet tab markup과 holdings breakdown CSS를 다시 복제하지 말 것.
- **passport positions tab canonical path**: positions 탭의 open/tracked/closed trade list는 `src/components/passport/PassportPositionsTab.svelte`가 canonical이다. route에서 positions list preview/overflow markup와 CTA empty state를 다시 인라인하지 말 것.
- **passport arena tab canonical path**: arena 탭의 learning panel mount, agent squad, match history surface는 `src/components/passport/PassportArenaTab.svelte`가 canonical이다. route에 arena match list/agent squad markup를 다시 복제하지 말 것.
- **passport tab shared CSS canonical path**: passport tab 공통 panel/detail styling은 `src/components/passport/passportTabShared.css`가 canonical이다. route `<style>`로 content-panel/detail-block 계열을 되돌리지 말 것.
...
- **Chart drawing runtime surface 최소화 유지**: `src/components/arena/chart/chartDrawingRuntime.ts`는 현재 shell이 실제로 쓰는 base drawing control만 노출한다. magnet toggle, import/export, selected-drawing style mutation, duplicate/lock helper 같은 admin-only wrapper를 다시 runtime public surface로 복구하면 `chartPanelSupportRuntime`가 불필요하게 커진다. 이런 기능이 다시 필요하면 usage를 확인한 뒤 별도 lazy admin runtime으로 분리할 것.
- **Chart drawing persistence runtime canonical path**: primitive drawing manager의 lazy module load, debounced autosave, pair/timeframe persistence flush/load sequencing은 `src/components/arena/chart/chartDrawingPersistenceRuntime.ts`가 단일 진실원이다. `chartDrawingRuntime.ts`나 `chartPanelSupportRuntime.ts`에 drawing manager import promise, autosave timer, pair/timeframe sync bookkeeping을 다시 흩뿌리지 말 것.
- **Chart drawing manager type canonical path**: primitive drawing stack의 serialized shape와 callback contract는 `src/lib/chart/primitives/drawingManagerTypes.ts`가 단일 진실원이다. `drawingManager.ts`, undo/persistence helper, runtime 파일에 `DrawingData`/`CandleOHLC`/`DrawingManagerCallbacks`를 다시 중복 선언하지 말 것.
- **Chart drawing primitive registry canonical path**: primitive class 선택, serialized drawing restore, primitive snapshot export는 `src/lib/chart/primitives/drawingPrimitiveRegistry.ts`가 단일 진실원이다. `drawingManager.ts`에 primitive subclass import switch, `toJSON()` 해석, restore/snapshot 분기를 다시 길게 인라인하지 말 것.
- **Chart position primitive lazy registry canonical path**: `PositionPrimitive` preview 생성과 serialized position restore는 `src/lib/chart/primitives/drawingPositionPrimitiveRegistry.ts`가 단일 진실원이다. `drawingPrimitiveRegistry.ts`에 `PositionPrimitive` 정적 import와 long/short entry preview constructor를 다시 합치면 base drawing registry chunk가 다시 커진다.
- **Chart trade-preview finalize lazy path**: `src/components/arena/chart/chartDrawingRuntime.ts`는 trade preview draft 시작/갱신과 mode reset만 즉시 들고, preview resolve/tradebox drawing/finalize side effect는 `src/components/arena/chart/chartTradePreviewFinalizeRuntime.ts`를 mouse-up 시점에 동적 import해서 처리한다. `chartDrawingRuntime.ts`에 `chartDrawingEngine`, `chartOverlayRenderer`, `chartDrawingSession`의 finalize 경로 import를 다시 정적으로 복구하면 `chartPanelSupportRuntime`가 다시 두꺼워진다.
...
- **Chart signal-assembly lazy boundary 유지**: community signal draft/evidence 조립에 필요한 `src/lib/chart/chartCommunitySignalDraft.ts`와 `src/lib/terminal/signalEvidence.ts`는 `src/components/arena/chart/chartActionRuntime.ts`에서 client action 시점에 동적 import한다. `buildCommunitySignalDraft(...)`를 다시 `chartTradePlanner.ts`에 합치거나 `chartActionRuntime.ts`가 정적 import로 되돌리면 reporter warning이 재발하고 `chartPanelSupportRuntime` 서버 청크가 다시 커진다.
- **Chart interactive action lazy path**: `src/components/arena/chart/chartActionRuntime.ts`는 pair/timeframe/scan처럼 즉시 필요한 action만 정적으로 들고, `chat assist`, `community signal publish`, `trade drawing activation`은 `src/components/arena/chart/chartInteractiveActionRuntime.ts`를 첫 user action에서만 동적 import한다. 이 interactive path를 다시 `chartActionRuntime.ts` 본체에 합치면 `chartPanelSupportRuntime`가 다시 두꺼워지고, 초기 차트 진입 경로에 optional action 비용이 섞인다.
...
- **Chart agent action overlay canonical path**: action-stack composition 경계와 child render order는 `src/components/arena/chart/ChartAgentActionOverlay.svelte`가 단일 진실원이다. trade/status HUD 조립과 action-level prop handoff를 다시 `ChartAgentOverlayChrome.svelte`에 합치지 말 것.
- **Chart agent trade HUD canonical path**: close button, first-scan CTA, trade CTA bar는 `src/components/arena/chart/ChartAgentTradeHud.svelte`가 단일 진실원이다. trade CTA DOM과 스타일을 다시 `ChartAgentActionOverlay.svelte`에 길게 인라인하지 말 것.
- **Chart agent status HUD canonical path**: drawing/position HUD composition 경계와 child render order는 `src/components/arena/chart/ChartAgentStatusHud.svelte`가 단일 진실원이다. drawing/position HUD 조립과 shared prop handoff를 다시 `ChartAgentActionOverlay.svelte`에 합치지 말 것.
- **Chart agent drawing status HUD canonical path**: drawing notice, chart notice, drag indicator는 `src/components/arena/chart/ChartAgentDrawingStatusHud.svelte`가 단일 진실원이다. drawing-status absolute-position DOM과 스타일을 다시 `ChartAgentStatusHud.svelte`에 길게 인라인하지 말 것.
- **Chart agent position HUD canonical path**: position sizer dock, position badge, risk-reward label surface는 `src/components/arena/chart/ChartAgentPositionHud.svelte`가 단일 진실원이다. position absolute-position DOM과 label formatting을 다시 `ChartAgentStatusHud.svelte`에 합치지 말 것.
- **Chart toolbar canonical path**: drawing toolbar의 open-category state, last-selected tool memory, standalone cursor/magnet/eraser/visibility/trash action wiring은 `src/components/arena/chart/ChartToolbar.svelte`가 단일 진실원이다. `ChartAgentSurface.svelte`나 `ChartPanel.svelte`에 toolbar state와 category markup를 다시 인라인하지 말 것.
- **Chart toolbar popover canonical path**: category tool list popup의 markup, hover/active styles, checkmark affordance는 `src/components/arena/chart/ChartToolbarPopover.svelte`가 단일 진실원이다. popover DOM과 스타일을 다시 `ChartToolbar.svelte`에 길게 인라인하지 말 것.
- **Chart toolbar icon canonical path**: drawing toolbar와 popover가 공유하는 SVG icon surface는 `src/components/arena/chart/ChartToolbarIcon.svelte`가 단일 진실원이다. 같은 icon switch를 toolbar/popup 양쪽에 복제하지 말 것.
- **Chart toolbar catalog canonical path**: tool category 목록, default label, icon mapping은 `src/components/arena/chart/chartToolbarCatalog.ts`가 단일 진실원이다. `ChartToolbar.svelte`와 popup child가 tool metadata를 따로 들지 말 것.
...
- **Chart surface optional-overlay lazy path**: `src/components/arena/chart/ChartAgentSurface.svelte`는 `pendingTradePlan`가 생길 때만 `ChartTradePlanOverlay.svelte`를, agent context menu가 실제로 열릴 때만 `ChartDrawingContextMenu.svelte`를 동적 import한다. 두 overlay를 다시 정적 import로 되돌리거나 `ChartPanel.svelte`가 직접 lazy 상태를 소유하면 `ChartAgentSurface` server chunk가 다시 커지고 optional surface 비용이 초기 진입 경로로 새어 나온다.
...
- **Chart panel state-context canonical path**: indicator series ref/pane index projection, data-runtime `getSeriesContext()` payload, indicator accumulator snapshot read는 `src/components/arena/chart/chartPanelStateContext.ts`가 단일 진실원이다. `ChartPanel.svelte`나 `chartPanelSupportRuntime.ts`에 동일 object shape를 다시 손으로 복제하면 pane/series drift가 다시 생긴다.
- **Chart panel runtime-options canonical path**: runtime bundle의 `position`, `tradingView`, `data`, `bindings` option shape와 TradingView state patch apply 경계는 `src/components/arena/chart/chartPanelRuntimeOptions.ts`가 단일 진실원이다. `ChartPanel.svelte` 안에 `setState` patch 분기, drag target 매핑, data-runtime/setError wiring, binding option object를 다시 길게 인라인하지 말 것.
- **Chart drawing-stack lazy boundary 유지**: primitive drawing manager와 persistence stack은 `src/components/arena/chart/chartDrawingRuntime.ts`에서 시작되는 drawing runtime 경계 안, 구체적으로 `src/components/arena/chart/chartDrawingPersistenceRuntime.ts`에서 동적 import한다. 초기 mount는 `syncDrawingPersistence()`로 persistence만 먼저 확인하고, 현재 pair/timeframe에 저장된 drawing이 있거나 manager가 이미 활성화된 경우에만 `drawingManager`를 hydrate한다. `src/components/arena/chart/chartPanelSupportRuntime.ts`에서 create 직후 unconditional prewarm을 다시 넣지 말고, `drawingManager`/`drawingPersistence`를 support-runtime 정적 import로 끌어올리지 말 것.
- **Chart drawing primitive lazy split 유지**: `src/components/arena/chart/chartDrawingPersistenceRuntime.ts`는 drawing manager를 깨울 때 `src/lib/chart/primitives/drawingPrimitiveRegistry.ts`도 함께 동적 import해서 factory/snapshot 로직을 registry chunk로 분리한다. primitive subclass import를 다시 `drawingManager.ts`로 되돌리거나 registry를 support-runtime 정적 import로 끌어올리면 `drawingManager.js`가 다시 비대해진다.
- **Chart deferred position-tool preload canonical path**: `src/lib/chart/primitives/drawingManager.ts`는 `longentry/shortentry` mode를 선택할 때 `src/lib/chart/primitives/drawingPositionPrimitiveRegistry.ts`를 내부에서 비동기 준비시키고, 준비 전 mouse-down은 manager 내부 pending queue로 보관한다. 이 preload/pending replay 로직을 `chartDrawingRuntime.ts`나 toolbar 컴포넌트에 다시 흩뿌리지 말 것.
...
- **ChartPanel client runtime assembly canonical path**: `chartMountRuntime`, `chartPanelController`, `chartPanelSupportRuntime`, `chartDerivativesRuntime`의 lazy import와 최초 instantiation 순서는 `src/components/arena/chart/chartClientRuntime.ts`가 단일 진실원이다. `src/components/arena/ChartPanel.svelte`에 module promise/cache, support-runtime hydrate, controller/derivatives 생성 순서를 다시 길게 인라인하지 말 것.
- **ChartPanel mount-state canonical path**: prepared mount 결과의 bootstrap series/pane ref 적용, shell state normalization 적용, MA period binding 주입은 `src/components/arena/chart/chartPanelMountState.ts`가 단일 진실원이다. `src/components/arena/ChartPanel.svelte`에 `applyBootstrapState()`/`applyPreparedMount()` 내부의 series ref assignment와 MA binding 생성 순서를 다시 길게 인라인하지 말 것.
