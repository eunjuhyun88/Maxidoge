...
- **Chart drawing-stack lazy boundary 유지**: primitive drawing manager와 persistence stack은 `src/components/arena/chart/chartDrawingRuntime.ts`에서 preload/sync 시점에 동적 import되고, `src/components/arena/chart/chartPanelSupportRuntime.ts`는 client create 직후 prewarm만 건다. `drawingManager`/`drawingPersistence`를 다시 support-runtime 정적 import로 끌어올리지 말 것.
...
- **Arena battle layout canonical path**: battle 단계의 상위 배치는 `src/components/arena/ArenaBattleLayout.svelte`가 소유하고, 좌측 chart rail은 `src/components/arena/ArenaChartRail.svelte`, 우측 battle sidebar는 `src/components/arena/ArenaBattleSidebar.svelte`가 단일 진실원이다. `arena/+page.svelte`에 전투 레이아웃 DOM/CSS를 다시 되돌리지 말고, battle-side UI를 수정할 때는 해당 child boundary 안에서만 조정할 것.
