...
- **Arena battle layout canonical path**: battle 단계의 chart/sidebar/reward/result/PvP/floating-word 마크업과 battle-only responsive CSS는 `src/components/arena/ArenaBattleLayout.svelte`가 단일 진실원이다. `arena/+page.svelte`에 전투 레이아웃 DOM과 관련 CSS 블록을 다시 대량으로 되돌리지 말 것.
