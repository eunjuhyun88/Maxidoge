...
- `src/components/arena/chart/ChartAgentStatusHud.svelte`: ChartPanel agent overlay action stack의 drawing notice/chart notice/position sizer/position badge/drag indicator child boundary
...
- **Chart agent status HUD canonical path**: drawing notice, chart notice, position sizer dock, position badge, drag indicator는 `src/components/arena/chart/ChartAgentStatusHud.svelte`가 단일 진실원이다. draw/position absolute-position DOM과 스타일을 다시 `ChartAgentActionOverlay.svelte`에 합치지 말 것.
