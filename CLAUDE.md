...
- `src/components/arena/chart/ChartAgentActionOverlay.svelte`: ChartPanel agent overlay의 CTA/drawing notice/chart notice/position/drag child boundary
...
- **Chart agent action overlay canonical path**: first-scan CTA, trade CTA bar, drawing notice, chart notice, position sizer dock, position badge, drag indicator는 `src/components/arena/chart/ChartAgentActionOverlay.svelte`가 단일 진실원이다. draw/position 관련 absolute-position DOM과 스타일을 다시 `ChartAgentOverlayChrome.svelte`에 합치지 말 것.
