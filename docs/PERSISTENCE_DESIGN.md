...
| 현재 key | v3 역할 | 변경 |
|---------|---------|------|
| `maxidoge_state` | 매치 상태 캐시 | Supabase primary → 로컬 캐시 |
| `maxidoge_agents` | 에이전트 stats 캐시 | user_agent_progress 캐시 |
| `maxidoge_wallet` | 지갑 연결 상태 | 유지 (로컬 전용, 보안) |
| `maxidoge_match_history` | 매치 히스토리 캐시 | arena_matches 캐시 |
| `maxidoge_quicktrades` | 포지션 캐시 | quick_trades 캐시 |
| `maxidoge_tracked` | 추적 시그널 캐시 | tracked_signals 캐시 |
| `maxidoge_community` | 커뮤니티 캐시 | community_posts 캐시 |
| `maxidoge_profile` | 프로필 캐시 | user_passports 캐시 |
| `maxidoge_pnl` | PnL 캐시 | lp_transactions 캐시 |
| (신규) `maxidoge_scans` | 스캔 탭 캐시 | terminal_scan_runs 캐시 |
| (신규) `maxidoge_chat` | 채팅 캐시 | agent_chat_messages 캐시 |
