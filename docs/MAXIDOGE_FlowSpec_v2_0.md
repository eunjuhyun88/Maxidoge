...
| 항목 | 값 / 조건 |
|------|----------|
| 세션 상태 | 미인증 (`maxidoge_session` 쿠키 없음 또는 만료) |
| 사용자 티어 | guest |
...
10. 클라이언트: `maxidoge_wallet` store 갱신, 사용자 티어 `connected`/`verified` 업데이트, 모달 닫힘

**After (사후 조건)**

| 항목 | 기대 값 |
|------|--------|
| 세션 쿠키 | `maxidoge_session` 발급됨 (HttpOnly) |
| 사용자 티어 | `connected` (지갑만) 또는 `verified` (이메일+지갑) |
| `maxidoge_wallet` store | `{ address, provider, connected: true }` 저장 |
| WalletModal | 닫힘 |
| 보호 API 접근 | 정상 200 응답 |
...
18. 클라이언트: `maxidoge_profile` store 초기화, `passportStore` 초기화, 모달 닫힘
...
29. 클라이언트: `maxidoge_tracked` store 낙관적 업데이트 → 서버 ID로 교체
...
38. 클라이언트: `maxidoge_quicktrades` 상태 갱신
...
42. 클라이언트: `maxidoge_signals`, `maxidoge_quicktrades` 동시 갱신
...
|-----------------|---------|-----------|
| `maxidoge_state` | 앱 전역 상태 (pair, phase 등) | store 변경 시 |
| `maxidoge_wallet` | 지갑 주소/연결 상태 | 인증 성공/로그아웃 |
| `maxidoge_profile` | 사용자 프로필 캐시 | 로그인/프로필 수정 |
| `maxidoge_match_history` | 과거 Arena 매치 기록 | result Phase 완료 |
| `maxidoge_quicktrades` | 퀵트레이드 목록/상태 | open/close 액션 |
| `maxidoge_tracked` | 추적 시그널 목록 | Track/convert 액션 |
| `maxidoge_pnl` | 누적 PnL 데이터 | 포지션 종료 시 |
| `maxidoge_signals` | 시그널 허브 데이터 | signals 페이지 진입 |
| `maxidoge_community` | 커뮤니티 포스트 캐시 | posts API 응답 |
| **`maxidoge_passport`** | Passport 메트릭 캐시 (v3) | 매치 결과/일배치 |
| **`maxidoge_agents`** | 에이전트 진행도 + Spec 해금 (v3) | 매치 결과/Spec 해금 |
| **`maxidoge_draft`** | 최근 드래프트 조합 캐시 (v3) | 드래프트 제출 시 |
| **`maxidoge_lp`** | LP 잔액 + 티어 캐시 (v3) | LP 변동 시 |
...
195. 클라이언트: 모든 `maxidoge_*` localStorage 키 삭제
